...
### 6.4 用户授权
  概述：创建的新用户是没有任何权限的，甚至连登录数据库的权 限都没有，需要为其指定相应的权限。给一个用户赋权限使用命令grant，回收权限使用命令revoke
  授权用法：1)grant [roleNmae] to [username] [with admin option]
        2)grant [privilege] on [tableName] to [userName] [with grant option]
  用法说明：1)roleName 角色名称或系统权限
              username 被授权的用户名
  ...
  回收权限用法：revoke [privilege] on [tableName] from [userName]
### 6.5 使用profile管理用户口令
...
####字符型
  ...
  varchar2(20) 变长 最大4000字符 节省空间，但查询速度慢
  clob 字符型大对象 最大4G
####数字类型
...
#### 7.8.8 约束部分小结
  ...
  - 一个列上可以有多个约束
  - 一个约束定义在多个列上--联合主键  - 定义约束有表级和列级两种形式
#### 7.9 给表起别名
...
- ceil(n):向下取整，返回大于或是等于n的最小__整数__
...
### 15.1 使用子查询插入数据
  ...
  用法：sql>insert into kkk(id,name,deptno)
...
### 15.2 使用子查询更新数据
  ...
  用法：sql>update emp set （job,sal,comm）=
